...
names(itemlist) <- paste("Tr",c(1:9), sep = "")#given name to each and every list
itemlist
## coerce into transactions
tdata3 <- as(itemlist, "transactions")#to make it the same format as Groceries
tdata3
summary(tdata3)
...
freqitems
inspect(freqitems)
rules0= apriori(tdata, parameter = list(conf = 0.5, minlen=1, maxlen=3))
inspect(rules0)
...
write.csv(as.data.frame(inspect(freqitems)),'freqitems1.csv')#save as csv file
write.csv(as.data.frame(inspect(rules0)),'rules0.csv')
