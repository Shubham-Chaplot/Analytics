...
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5))#support suggest that 0.01*transactions must be done for it to be valid group
#length is the number of items in a group, here we want minimum number of items to be 2 and maximum number of items to be 5
inspect(frequentItems)# count is the number of times the group is present in the given data
frequentItems
...
rules#120 rules have been created
inspect (rules[1:5])#A is lhs and B is rhs, A is baking powder and B is whole milk, for second time A is other vegetable and oil and B is whole milk
...
rules3 = apriori (data=Groceries, parameter=list (supp=0.002,conf = 0.8), appearance = list (default="lhs",rhs="whole milk"), control = list (verbose=F))#rhs is whole milk and lhs is any item which matches with rhs
#using verbose=F set item appearances, set transactions, etc are not displayes
inspect(rules3[1:5])
inspect(rules3)

#Find out what events were influenced by a given event
subset1 = subset(rules2, appearance = list (default="lhs",rhs="whole milk"))#subset os rules where rhs is whole milk
subset1 = subset(rules2, subset=rhs %in% 'bottled beer' )
inspect(subset1)
inspect(rules2)
subset2 = subset(rules2, subset=lhs %ain% c('baking powder','soda') )#this is both baking powder and soda
inspect(subset2)
subset2a = subset(rules2, subset=lhs %in% c('baking powder','soda') )#this is either of two(baking and soda) is used
inspect(subset2a)
