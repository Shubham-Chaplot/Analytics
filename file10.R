...
plot(women, type='l')
plot(women, type='b', pch=18, lty=2, col=2)#col is colour
...
plot(women)
abline(lm(women$weight ~ women$height), col='red', lty=2, lwd=4)#line of regression


#boxplot
boxplot(women$height)
abline(h=c(58, 62,65,68,72))#msm
...
hist(x, freq=F, col=1:5)#merge the curve the normal distribution curve
lines(density(x))
...
cor(mtcars)
pairs(mtcars)
options(digits=4)#no. of decimal places
